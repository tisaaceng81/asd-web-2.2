...
def parse_edo(edo_str, entrada_str, saida_str):
    ...
    try:
        ...
        if Fs not in resto.free_symbols and resto != 0:
            raise ValueError(f"Não foi possível isolar a Função de Transferência. Termos remanescentes inesperados: {resto}. Verifique se a EDO é linear e se a variável de entrada está presente e correta.")
